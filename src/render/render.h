...
struct Color {
    ...
    explicit Color(float setR, float setG, float setB) : r(setR), g(setG), b(setB) {}
};
...
struct Vect3 {
    ...
    explicit Vect3(double setX, double setY, double setZ) : x(setX), y(setY), z(setZ) {}
...
struct Car {
    ...
    explicit Car(const Vect3& setPosition, const Vect3& setDimensions, const Color& setColor, float setVelocity,
                 float setAngle, float setLf, const std::string& setName)
    ...
    static Eigen::Quaternionf getQuaternion(float theta) {
        ...
        rotation_mat << std::cos(theta), -std::sin(theta), 0, std::sin(theta), std::cos(theta), 0, 0, 0, 1;
