        ...
        pcl::visualization::PCLVisualizer::Ptr& viewer)
	{
		...
		if(visualize_pcd)
		{
			...
			renderPointCloud(
                ...
                trafficCloud,
                "trafficCloud",
                Color(184.f/256, 223.f/256, 252.f/256));
		}
		...
		for (std::size_t i = 0; i < traffic.size(); ++i)
		{
			...
			if(trackCars[i])
			{
				...
				gt <<
                    traffic[i].position.x,
                    traffic[i].position.y,
                    traffic[i].velocity*std::cos(traffic[i].angle),
        ...
        Eigen::VectorXd rmse = tools.CalculateRMSE(
            tools.estimations,
            tools.ground_truth);
