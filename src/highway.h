...
public:
    ...
    explicit Highway(pcl::visualization::PCLVisualizer::Ptr& viewer)
        : tools(), egoCar(Vect3(0, 0, 0), Vect3(4, 2, 2), Color(0, 1, 0), 0, 0, 2, "egoCar") {
    ...
    void stepHighway(double egoVelocity, long long timestamp, int frame_per_sec,
        ...
        if (visualize_pcd) {
            ...
            renderPointCloud(viewer, trafficCloud, "trafficCloud", Color(184.f / 256, 223.f / 256, 252.f / 256));
        }
        ...
        for (std::size_t i = 0; i < traffic.size(); ++i) {
            ...
            if (trackCars[i]) {
                ...
                gt << traffic[i].position.x, traffic[i].position.y, traffic[i].velocity * std::cos(traffic[i].angle),
                    traffic[i].velocity * std::sin(traffic[i].angle);

                tools.ground_truth.emplace_back(gt);
                ...
                tools.estimations.emplace_back(estimate);
            }
        }
