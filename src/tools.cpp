...
lmarker Tools::lidarSense(Car& car, pcl::visualization::PCLVisualizer::Ptr& viewer, long long timestamp,
    ...
    const lmarker marker =
        lmarker(car.position.x + noise(0.15, timestamp), car.position.y + noise(0.15, timestamp + 1));
...
rmarker Tools::radarSense(Car& car, const Car& ego, pcl::visualization::PCLVisualizer::Ptr& viewer, long long timestamp,
                          bool visualize) {
    double rho = std::sqrt((car.position.x - ego.position.x) * (car.position.x - ego.position.x) +
    ...
    double rho_dot = (car.velocity * std::cos(car.angle) * rho * std::cos(phi) +
                      car.velocity * std::sin(car.angle) * rho * std::sin(phi)) /
                     rho;
    ...
    if (visualize) {
        ...
        viewer->addLine(pcl::PointXYZ(ego.position.x, ego.position.y, 3.0), pcl::PointXYZ(posX, posY, 3.0), 1, 0, 1,
                        car.name + "_rho");

        viewer->addArrow(pcl::PointXYZ(posX, posY, 3.0),
                         pcl::PointXYZ(posX + marker.rho_dot * std::cos(marker.phi),
                                       posY + marker.rho_dot * std::sin(marker.phi), 3.0),
...
void Tools::ukfResults(Car& car, pcl::visualization::PCLVisualizer::Ptr& viewer, double time, int steps) {
    ...
    viewer->addArrow(
        ...
        pcl::PointXYZ(ukf.x_[0] + ukf.x_[2] * std::cos(ukf.x_[3]), ukf.x_[1] + ukf.x_[2] * std::sin(ukf.x_[3]), 3.5), 0,
        1, 0, car.name + "_ukf_vel");
